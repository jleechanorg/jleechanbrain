...
if ! hermes memory index --force > "$OUT/reindex.log" 2>&1; then
...
for i, e in enumerate(expected, 1):
    ...
    for attempt in range(1, 4):
        ...
        try:
            p = subprocess.run(
                ['hermes', '--log-level', 'fatal', 'agent', '--local', '--agent', agent_id, '--session-id', session_id, '--timeout', '120', '--json', '--thinking', 'off', '-m', prompt],
                capture_output=True,
                text=True,
                env=os.environ.copy(),
                timeout=140,
