#!/usr/bin/env bash
# install-hermes-launchd.sh
# Central entrypoint for installing all Hermes launchd services.
# Single source of truth for install. Called by bootstrap.sh and run standalone.
#
# Installs:
#   Infrastructure (always-on daemons):
#     com.smartclaw.gateway        — Hermes gateway (port 18789)
...
#   Gateway-cron jobs are managed by the Hermes gateway itself and are NOT
...
#   ./scripts/install-hermes-launchd.sh --dry-run
#
set -euo pipefail
...
if [[ -x "$_sched" ]]; then
  ...
  echo "  ✗ install-hermes-scheduled-jobs.sh not found: $_sched" >&2
fi
...
if [[ ! -f "$_watchdog_plist" ]] && [[ -f "$_watchdog_template" ]]; then
  # Substitute @HOME@ with actual $HOME in plist before deploying
  sed "s|@HOME@|$HOME|g" "$_watchdog_template" > "$_watchdog_plist"
