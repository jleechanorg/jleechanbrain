...
LOOKBACK_HOURS=24
REPORT_CHANNEL="${STABILITY_REPORT_CHANNEL:-C0AJQ5M0A0Y}"  # #ai-general — Jeffrey asked 2026-07-16 to move out of #all-jleechan-ai
...
resolve_token() {
  local tok=""
  tok="$(cat "$HOME/.smartclaw/config.yaml" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty' 2>/dev/null)" || true
  tok="${tok:-$GH_TOKEN}"
...
if [[ -n "$SLACK_TOKEN" ]]; then
  log "Posting to Slack channel $REPORT_CHANNEL (threaded under daily anchor)..."
  # slack_post handles thread-anchor persistence + dedupe + channel resolution.
  # Net LOC: -10 lines (the inline curl was 18 lines; one call replaces it).
  export SLACK_BOT_TOKEN="$SLACK_TOKEN"
  slack_post "stability-report" "$SLACK_MESSAGE" --channel "$REPORT_CHANNEL" 2>&1 | log || true
else
