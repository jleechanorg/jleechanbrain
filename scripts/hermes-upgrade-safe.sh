#!/usr/bin/env bash
# hermes-upgrade-safe.sh
# Safe wrapper around `npm install -g @hermesai/hermes@<version>` that validates
# SDK compatibility and runs pre-flight checks before upgrading.
#
# Usage:
#   bash hermes-upgrade-safe.sh <new-version>               # interactive confirm
#   bash hermes-upgrade-safe.sh <new-version> --yes         # non-interactive
#   bash hermes-upgrade-safe.sh <new-version> --ack-minor-jump  # allow 0.x SDK jump
#   bash hermes-upgrade-safe.sh <new-version> --yes --ack-minor-jump
#
# Example:
#   bash hermes-upgrade-safe.sh 2026.4.1
set -uo pipefail
...
if [ -z "$NEW_VERSION" ]; then
  echo "Usage: $(basename "$0") <new-hermes-version> [--yes] [--ack-minor-jump]"
  echo "Example: $(basename "$0") 2026.4.1"
  exit 1
fi

echo "=== Hermes Safe Upgrade: $NEW_VERSION ==="
echo ""
...
if [ -f "$PREFLIGHT" ]; then
  ...
  _new_sdk=$("$GATEWAY_NPM" view "hermes@${NEW_VERSION}" dependencies 2>/dev/null \
    | grep -i agentclientprotocol | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || true)
  _new_sdk="${_new_sdk:-unknown}"

  echo "  Current hermes @agentclientprotocol/sdk : ${_cur_sdk}"
  echo "  Target  hermes @agentclientprotocol/sdk : ${_new_sdk}"
  ...
  if [ "$_new_sdk" = "unknown" ] || [ "$_cur_sdk" = "unknown" ]; then
    ...
    if [ "$_major_diff" -gt 0 ]; then
      ...
      echo "  Incident ref: hermes 2026.3.24→2026.3.28 (SDK 0.16→0.17) → 367 ws-stream 500s/day"
      _sdk_ok=0
...
_current_version=$("$GATEWAY_NPM" list -g hermes --depth=0 --json 2>/dev/null \
  | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('dependencies',{}).get('hermes',{}).get('version','(not installed)'))" 2>/dev/null \
  || echo "(not installed)")
...
# --- Step 5a: Backup config.yaml ---
echo "--- Step 5/5: Backup + upgrade ---"
_backup="$HOME/.smartclaw/config.yaml.pre-upgrade-$(date +%s)"
if [ -f "$HOME/.smartclaw/config.yaml" ]; then
  cp "$HOME/.smartclaw/config.yaml" "$_backup"
  echo "  Backed up config.yaml → $(basename "$_backup")"
fi

# --- Step 5b: Run upgrade ---
echo "  Running: $GATEWAY_NPM install -g hermes@${NEW_VERSION}"
"$GATEWAY_NPM" install -g "hermes@${NEW_VERSION}" || {
  echo ""
  echo "UPGRADE FAILED — npm install returned non-zero"
  echo "config.yaml backup preserved at: $_backup"
  exit 1
}
...
echo "=== Upgrade complete: hermes@${NEW_VERSION} ==="
echo "Run 'hermes gateway status' to verify the gateway is healthy."
