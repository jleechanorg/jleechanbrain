...
# Safe wrapper around `npm install -g hermes@<version>` that validates
...
# --- Step 5a: Backup hermes.json ---
echo "--- Step 5/5: Backup + upgrade ---"
_backup="$HOME/.smartclaw/hermes.json.pre-upgrade-$(date +%s)"
if [ -f "$HOME/.smartclaw/hermes.json" ]; then
  cp "$HOME/.smartclaw/hermes.json" "$_backup"
  echo "  Backed up hermes.json → $(basename "$_backup")"
fi
...
"$GATEWAY_NPM" install -g "hermes@${NEW_VERSION}" || {
  ...
  echo "hermes.json backup preserved at: $_backup"
  exit 1
}
