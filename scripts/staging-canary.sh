...
# Run against staging (port 8644) before applying changes to production (8643).
...
PROD_PORT="${HERMES_PROD_PORT:-8643}"
...
if [[ -n "${HERMES_STAGING_CONFIG:-}" ]]; then
    ...
    echo "  WARN: Unknown port $PORT — defaulting to staging config (~/.smartclaw/config.staging.yaml)"
    CONFIG_FILE="$HOME/.smartclaw/config.staging.yaml"
fi
...
NODE_BIN="${HERMES_NODE_BIN:-$(launchctl print gui/$(id -u)/ai.smartclaw.prod 2>/dev/null | grep -oE '/[^ ]*bin/node' | head -1 || echo '${HOME}/.nvm/versions/node/v22.22.0/bin/node')}"
...
# Resolve \${VAR} placeholders (config.yaml stores tokens as env references)
...
if [[ -z "$SDK_VERSION" ]]; then
    ...
    SDK_VERSION=$(node -e "try { const p = require(require.resolve('@agentclientprotocol/sdk/package.json', {paths:['/opt/homebrew/lib/node_modules/hermes','${HOME}/.nvm/versions/node/v22.22.0/lib/node_modules/hermes','$(echo ~)/.smartclaw']})); console.log(p.version); } catch(e) { console.log(''); }" 2>/dev/null || echo "")
fi
if [[ -z "$SDK_VERSION" ]]; then
    ...
    elif [[ "$SDK_MINOR" -le 20 ]]; then
        check "SDK protocol version" 0 "SDK $SDK_VERSION (protocol compat — major=0, minor <= 20)"
    else
        check "SDK protocol version" 1 "SDK $SDK_VERSION (protocol 0.${SDK_MINOR} — minor > 20, breaks ws-stream)"
    fi
fi
...
# Must filter by port (lsof) not just pgrep — staging (8644) and prod (8643)
