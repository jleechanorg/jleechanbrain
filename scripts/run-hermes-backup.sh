...
BACKUP_SCRIPT="$SCRIPT_DIR/backup-hermes-full.sh"
LOG_DIR="${HOME}/Library/Logs/hermes-backup"
LOCK_FILE="$LOG_DIR/hermes-backup.lock"
LOG_FILE="$LOG_DIR/hermes-backup.log"
mkdir -p "$LOG_DIR"

SMTP_SERVER="smtp.gmail.com"
SMTP_PORT="587"
EMAIL_FROM="hermes-backup@worldarchitect.ai"
...
send_failure_email() {
  ...
  local report_file="$LOG_DIR/hermes-backup-failure-${failure_ts}.txt"

  cat <<EOF > "$report_file"
Subject: ALERT: Hermes backup failed on $(hostname) (${timestamp})
From: $EMAIL_FROM
To: ${BACKUP_EMAIL:-backup-alerts@worldarchitect.ai}

Hermes backup wrapper failure.
...
EOF
  ...
  mkdir -p "$manual_dir"
  cp "$report_file" "$manual_dir/hermes_backup_failure_${failure_ts}.txt"
  echo "[$(TS)] Failure alert could not be sent; report saved to $manual_dir/hermes_backup_failure_${failure_ts}.txt"
  return 0
}
...
if [ "${HERMES_BACKUP_FORCE_FAILURE:-0}" = "1" ]; then
  echo "[$(TS)] Forced failure enabled for verification: HERMES_BACKUP_FORCE_FAILURE=1" >> "$LOG_FILE"
  send_failure_email 1 "Forced failure test (HERMES_BACKUP_FORCE_FAILURE=1)" >> "$LOG_FILE" 2>&1
  echo "[$(TS)] Backup failed (exit=1): Forced failure test (HERMES_BACKUP_FORCE_FAILURE=1)"
  exit 1
fi
