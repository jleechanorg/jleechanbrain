...
ensure_agent_orchestrator_runtime_config() {
    ...
    if [[ ! -f "$canon_yaml" && ! -f "$repo_yaml" ]]; then
        ...
        echo "Skipping ~/agent-orchestrator.yaml: neither $canon_yaml nor $repo_yaml exists (hermes)"
        return 0
    fi
    local source_yaml
...
# --- Install hermes CLI (prefer Homebrew Node over NVM) ---
echo ""
...
if command -v node >/dev/null 2>&1; then
    ...
    if command -v hermes >/dev/null 2>&1; then
        ...
        echo "  To upgrade: npm install -g @nousresearch/hermes@latest"
    else
        echo "  Installing hermes globally via npm..."
        npm install -g @nousresearch/hermes@latest
        if command -v hermes >/dev/null 2>&1; then
...
echo ""
echo "Next: inject real tokens into config.yaml"
echo "  # create or edit config.yaml with real tokens"
echo ""
