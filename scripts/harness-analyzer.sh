...
# Daily 9am job that analyzes jleechanbrain for harness engineering violations
...
*Generated by harness-analyzer.sh - daily 9am job*"
    ...
    "$GH_SAFE_PUBLISH" pr create --title "fix: harness engineering improvements ($(date +%Y-%m-%d))" \
        --body "$pr_body" \
...
*Automated review by harness-analyzer.sh*"
            ...
            "$GH_SAFE_PUBLISH" pr comment "$pr_num" --body "$comment_body" --repo "$REPO" 2>/dev/null
