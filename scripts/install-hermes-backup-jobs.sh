...
RUNNER="$SCRIPTS/run-hermes-backup.sh"
WATCHDOG="$SCRIPTS/backup-watchdog.sh"
PLIST_TEMPLATE="$SCRIPTS/hermes-backup.plist.template"
LAUNCHD_DIR="$HOME/Library/LaunchAgents"
PLIST_DST="$LAUNCHD_DIR/com.smartclaw.backup.plist"
CRON_MARKER="# Hermes 4h backup for ~/.smartclaw"
WATCHDOG_MARKER="# Hermes backup watchdog (hourly)"
DROPBOX_MARKER="# Hermes Dropbox backup (4h, offset :10)"
DROPBOX_BACKUP="$SCRIPTS/dropbox-hermes-backup.sh"

mkdir -p "$LAUNCHD_DIR"
mkdir -p "$HOME/Library/Logs/hermes-backup"
...
if command -v crontab >/dev/null 2>&1; then
  ...
  if [[ -s "$CRON_BEFORE" ]]; then
    ...
    if ! cmp -s "$CRON_BEFORE" "$CRON_AFTER"; then
      crontab "$CRON_AFTER"
      echo "Removed legacy Hermes backup entries from system crontab."
    fi
  fi
...
echo "Done. Hermes backup scheduling now uses launchd only."
