...
# - Required: launchd workflow (`hermes-config/ai.smartclaw.schedule.*.plist` + install scripts).
...
check_file_for_violation() {
  ...
  while IFS=: read -r line_no line; do
    ...
    if (echo "$context" | rg -iq 'crontab' && echo "$context" | rg -iq 'hermes|\.smartclaw|ai\.smartclaw|hermes-backup|backup-content') \
      || echo "$context" | rg -iq '\bhermes\s+cron\b'; then
      echo "Guardrail violation: $file:$line_no"
      echo "  $line"
      violations=1
    fi
  done < <(rg -n --no-heading -i 'crontab|\bhermes\s+cron\b' "$file" || true)
...
while IFS= read -r file; do
  case "$file" in
    .smartclaw-backups/*|hermes/.smartclaw-backups/*|hermes-config/agents/*|hermes-config/credentials/*|scripts/check-hermes-cron-guardrail.sh|scripts/install-hermes-backup-jobs.sh|scripts/setup-hermes-full.sh|BACKUP_AND_RESTORE.md|CLAUDE.md|testing_llm/MEMORY_QUALITY_TEST.md)
      continue
      ;;
  esac
