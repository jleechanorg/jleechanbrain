...
#
# NOTE: Phase 2 (install) is NOT automated — templates in ~/.smartclaw/launchd/
# use @HOME@/@HERMES_BIN@/@REPO_ROOT@ placeholders that require substitution.
# The existing hermes plists in ~/Library/LaunchAgents/ are already correctly
# deployed. Only stale openclaw/agento-era plists need removal.
...
if $STATUS_ONLY; then
  ...
  ls "$LAUNCHD_DIR" | grep -E "hermes|agento|mctrl|agentorchestrator" | sort
  exit 0
fi
...
for plist_file in "${STALE_PLISTS[@]}"; do
  ...
  label=$(python3 -c "import plistlib; d=plistlib.load(open('$full_path','rb')); print(d.get('Label',''))" 2>/dev/null || echo "")
...
# SAFETY: Only install if source has NO @PLACEHOLDER@ tokens.
# Templates with @HOME@/@HERMES_BIN@/@REPO_ROOT@ must be substituted before
# installing. Overwriting live plists with raw templates breaks the gateway.
echo ""
...
CORE_PLISTS=(
  ...
  "ai.smartclaw-mem0-server.plist"
)
...
_install_plist() {
  ...
  local label
  label=$(python3 -c "import plistlib; d=plistlib.load(open('$src','rb')); print(d.get('Label',''))" 2>/dev/null || echo "")
  ...
  HERMES_BIN_PATH=$(readlink -f /opt/homebrew/bin/hermes 2>/dev/null || echo /opt/homebrew/bin/hermes)
  sed \
    -e "s|@HOME@|$HOME|g" \
...
if ! $DRY_RUN; then
  ...
  # Check prod gateway
  GW_PID=$(launchctl list 2>/dev/null | grep "ai.smartclaw.prod$" | awk '{print $1}')
  PORT_PID=$(lsof -t -i :8642 2>/dev/null || echo "")

  if [ -n "$GW_PID" ] && [ "$GW_PID" != "-" ]; then
    if [ "$PORT_PID" = "$GW_PID" ]; then
      ok "  Gateway: PID=$GW_PID bound to :8642 ✓"
    else
      warn "  Gateway: PID=$GW_PID but port 8642 bound by PID=${PORT_PID:-none}"
    fi
  else
