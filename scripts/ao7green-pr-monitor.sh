...

{
  ...
  resolve_token() {
    local tok=""
    tok="$(cat "$HOME/.smartclaw/config.yaml" 2>/dev/null | python3 -c "import sys,yaml; d=yaml.safe_load(sys.stdin); print(d.get('skills',{}).get('entries',{}).get('gh-issues',{}).get('apiKey',''))" 2>/dev/null)" || true
    tok="${tok:-$GH_TOKEN}"
