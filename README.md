...
jleechanclaw is the **control layer**. It turns plain-English requests into reliable engineering execution:

1. You ask in Slack (or another connected channel).
2. jleechanclaw expands context (thread history, policy, constraints, memory).
...
5. jleechanclaw reports back with proof artifacts (PR URL, commit URL, checks).

### jleechanclaw + Agent Orchestrator (exact repo pairing)
...
- Hermes notifier plugin: `packages/plugins/notifier-hermes/src/index.ts`
...
In short: **jleechanclaw handles intent, policy, and user communication**; **Agent Orchestrator handles parallel execution, worktree isolation, and PR/CI feedback loops**.
...
```bash
git clone https://github.com/jleechanorg/jleechanclaw.git ~/.hermes
cd ~/.hermes
# Create config.yaml with your real tokens (no redacted template is committed)
...
2. Verify `config.yaml` exists and has real tokens
...
```bash
# ── Slack identity ────────────────────────────────────────────────────────────
export HERMES_SLACK_BOT_TOKEN="xoxb-..."          # from Slack app OAuth & Permissions
export HERMES_SLACK_APP_TOKEN="xapp-..."          # optional — Socket Mode only
...
export HERMES_BOT_USER_ID="U0AEZC7RX1Q"          # hermes bot user ID
...
export HERMES_MONITOR_SLACK_TARGET="C0AP8LRKM9N"      # canary/monitor alert channel
export HERMES_MONITOR_THREAD_REPLY_CHANNEL="C0AP8LRKM9N"

# ── Gateway ───────────────────────────────────────────────────────────────────
export HERMES_URL="http://127.0.0.1:8643"
export HERMES_GATEWAY_TOKEN="<token>"             # from config.yaml
export HERMES_AO_HOOK_TOKEN="<token>"             # from config.yaml hooks.token (or HERMES_HOOKS_TOKEN)
```
...
|----------|----------|-------------|
| `HERMES_SLACK_BOT_TOKEN` | Yes | Slack bot token (`xoxb-…`). Used by monitor and notifier to post alerts. |
| `HERMES_SLACK_APP_TOKEN` | Optional | Slack app-level token (`xapp-…`). Used for Socket Mode if enabled. |
...
| `HERMES_BOT_USER_ID` | `U0AEZC7RX1Q` | Hermes bot user ID. Gateway ignores its own messages using this. |
...
| `HERMES_MONITOR_SLACK_TARGET` | monitor canary channel | `C0AP8LRKM9N` | Primary alert channel for monitor problem reports |
| `HERMES_MONITOR_PROBE_SLACK_TARGET` | same | same | Probe channel for gateway Slack connectivity |
| `HERMES_MONITOR_GATEWAY_PROBE_TARGET` | same | same | Channel monitor sends probe messages to |
| `HERMES_MONITOR_GATEWAY_PROBE_MESSAGE_ENABLE` | `0` | n/a | Set to `1` to post startup "monitor check started" probe; default `0` keeps startup probe silent |
| `HERMES_MONITOR_STATUS_SLACK_TARGET` | monitor canary channel | `C0AP8LRKM9N` | Status-broadcast — receives periodic health summary |
| `HERMES_MONITOR_THREAD_REPLY_CHANNEL` | same | same | Scanned for open threads lacking a bot reply |
...
| Variable | Required | Description |
|----------|----------|-------------|
| `HERMES_AO_HOOK_TOKEN` | Yes | Webhook token for the AO → hermes notifier. Set this so `agent-orchestrator.yaml` can authenticate to the local hermes gateway at `http://127.0.0.1:8643/hooks/agent`. Get the value from the hermes gateway config: `config.yaml` → `hooks.token` (commonly exposed as `HERMES_HOOKS_TOKEN`), or reuse a previously generated token. |

Tracked source config lives in this repo as `agent-orchestrator.yaml`. `bootstrap.sh` renders a local runtime copy at `~/.agent-orchestrator.yaml` with shell placeholders resolved from your environment and points `~/agent-orchestrator.yaml` at that rendered file for AO compatibility.

Example:
```bash
# ~/.bashrc
export HERMES_AO_HOOK_TOKEN="<token>"
```
...
|----------|---------|-------------|
| `HERMES_MONITOR_HTTP_GATEWAY_URL` | `http://127.0.0.1:8643/health` | URL for the HTTP health-check probe against the local gateway. |
| `HERMES_MONITOR_SLACK_API_BASE` | `https://slack.com/api` | Base URL for Slack API calls (override for testing). |
| `HERMES_MONITOR_CANARY_TIMEOUT_SECONDS` | `45` | How long to wait for a canary Slack round-trip before declaring failure. |
| `HERMES_MONITOR_CANARY_POLL_INTERVAL_SECONDS` | `3` | Polling interval while waiting for canary response. |
| `HERMES_MONITOR_RUN_CANARY` | `1` | Slack canary probe enabled by default. Sender token precedence: `HERMES_MONITOR_CANARY_BOT_TOKEN` → `~/.mcp_mail/credentials.json:SLACK_BOT_TOKEN` → `SLACK_USER_TOKEN` (legacy fallback). Set to `0` to disable. |
| `HERMES_MONITOR_CANARY_BOT_TOKEN` | _(unset)_ | Optional dedicated bot token for canary sender identity (recommended for bot-to-bot canary checks). |
| `HERMES_MONITOR_PHASE1_REMEDIATION_ENABLE` | `1` | Set to `0` to disable Phase 1 auto-remediation (gateway restart). |
| `HERMES_MONITOR_PHASE2_ENABLE` | `1` | Set to `0` to disable Phase 2 deep-diagnosis. |
| `HERMES_MONITOR_PHASE2_AUTOFIX_ENABLE` | `1` | Set to `0` to prevent Phase 2 from applying auto-fixes. |
| `HERMES_MONITOR_PHASE2_ALLOW_CONFIG_MUTATIONS` | `0` | Set to `1` to allow Phase 2 to mutate hermes config files. |
| `HERMES_MONITOR_PHASE2_TIMEOUT_SECONDS` | `120` | Max seconds Phase 2 may run before being killed. |
| `HERMES_MONITOR_STATUS_BROADCAST_ENABLE` | `1` | Set to `0` to suppress the periodic status broadcast. |
| `HERMES_MONITOR_THREAD_REPLY_CHECK` | `1` | Set to `0` to disable the open-thread reply check. |
| `HERMES_MONITOR_THREAD_REPLY_LOOKBACK_SECONDS` | `21600` | How far back (in seconds) to look for unanswered threads (default 6 h). |
| `HERMES_MONITOR_THREAD_REPLY_GRACE_SECONDS` | `120` | Grace period after a message is posted before it counts as unanswered. |
| `HERMES_MONITOR_THREAD_REPLY_MAX_THREADS` | `12` | Max threads to inspect per run. |
| `HERMES_MONITOR_THREAD_REPLY_FAILURE_REGEX` | (error patterns) | Extended-regex matched against thread messages to detect failure signatures. |
| `HERMES_MONITOR_THREAD_REPLY_FAILURE_MAX_AGE_SECONDS` | `900` | Threads older than this (15 min) with failure patterns are suppressed. |
| `HERMES_MONITOR_BOT_USER_ID` | _(auto-detected)_ | Slack user ID of the hermes bot. Used to check whether the bot has already replied. |
| `HERMES_MONITOR_LOCK_STALE_SECONDS` | `7200` | Lock file is considered stale after this many seconds (allows a stuck run to be superseded). |
| `HERMES_MONITOR_DOCTOR_SH_ENABLE` | `1` | Set to `0` to skip running `scripts/doctor.sh` during monitoring. |
| `HERMES_MONITOR_DOCTOR_SH_ALWAYS` | `1` | Set to `0` to run doctor only when a problem is detected. |
| `HERMES_MONITOR_DOCTOR_SH_PATH` | _(auto-detected)_ | Path to `doctor.sh`. Override if the script is not at its default location. |
...
|-----------|--------------|---------|
| [Hermes](https://github.com/openclaw/openclaw) | Agent runtime — persistent memory, channels, gateway | `npm install -g hermes` |
...
|--------|--------------|---------|
| [hermes-mem0](https://github.com/jleechanorg/openclaw-mem0) | Long-term semantic memory via Qdrant + mem0 — auto-captures and recalls context across sessions | `hermes plugins install hermes-mem0` (already bundled in extensions/) |
| [lossless-claw](https://github.com/martian-engineering/lossless-claw) | Replaces sliding-window context truncation with hierarchical DAG summarization — all messages persist in SQLite (`~/.hermes/lcm.db`), compressed via LLM when context fills. Exposes `lcm_grep`, `lcm_describe`, `lcm_expand` tools so agents can search full history | `hermes plugins install --link ~/projects_reference/lossless-claw` (linked local clone) |
...
| Qdrant | Vector DB for hermes-mem0 semantic memory | LaunchAgent: `ai.hermes.qdrant` |
...
| `config.yaml` | Live local gateway config (gitignored; must contain real tokens and runtime settings) |
...
| Script | Purpose |
|--------|---------|
| `backup-hermes-full.sh` | Full recursive backup of `~/.hermes/` with secret redaction |
| `run-hermes-backup.sh` | Backup runner with locking and failure alerts |
| `dropbox-hermes-backup.sh` | Dropbox-targeted backup |
| `install-hermes-backup-jobs.sh` | Install launchd plist for scheduled backups |
| `check-hermes-cron-guardrail.sh` | CI guardrail: launchd-only scheduling for repo-managed Hermes jobs |
| `setup-hermes-full.sh` | Full first-time Hermes setup |
| `install-launchagents.sh` | Install all hermes launchd plists from root directory |
...
| `bootstrap-hermes-config.sh` | Bootstrap fresh Hermes configuration |
...
| `hermes-backup-jobs.md` | Backup job documentation |
...
| `ORCHESTRATION_DESIGN.md` | Orchestration system design (AO + Hermes) |
...
Standalone Hermes agent config for a Discord engineering bot — separate `config.yaml` and `SOUL.md` for a Discord-channel-native agent persona.
...
| Module | Purpose |
|--------|---------|
| `backup_redaction.py` | Secret redaction for `~/.hermes/` backups before offsite copy |
...
Changes to `~/.hermes/` (live gateway config) go through a 3-stage safety pipeline before reaching production:

1. **Staging branch** — PRs target `staging` (not `main`); `~/.hermes-staging/` is a git worktree
2. **Canary** — `scripts/staging-canary.sh --port 8644` runs 6 health checks against staging gateway
...
| Gateway health, Slack connectivity, native module ABI, heartbeat | Self-hosted CI (`staging-canary-full.yml`) or manual (`staging-canary.sh --port 8645` / `8644`) |
...
- automatic sanitization of org/user/path/channel tokens (`jleechan*`, `/Users/jleechan`, `.hermes`)

Never exported:
- live config/secrets (`config.yaml*`)
...
- Setting up `~/.hermes/cron/jobs.json` (lives in live `~/.hermes/`, not in this repo)
- Configuring the gateway's cron scheduler

## Launchd Scheduled Jobs (repo-managed)

Repo-managed recurring jobs run via launchd labels `ai.hermes.schedule.*`.
Plist files are in the repo root (`ai.hermes.schedule.*.plist`).

> **Note**: Scheduled job payloads are managed in the live `~/.hermes/` directory, not in this repository. See `AUTO_START_GUIDE.md` for setting up launchd services.

| Plist | Purpose |
|-------|---------|
| `ai.hermes.schedule.backup-4h20.plist` | Backup every 4 hours |
| `ai.hermes.schedule.daily-checkin-9am.plist` | Morning check-in |
| `ai.hermes.schedule.daily-checkin-12pm.plist` | Midday check-in |
| `ai.hermes.schedule.daily-checkin-6pm.plist` | Evening check-in |
| `ai.hermes.schedule.genesis-memory-curation-weekly.plist` | Weekly memory curation |
...
./scripts/install-hermes-backup-jobs.sh  # set up scheduled backups
```
...
./scripts/install-hermes-scheduled-jobs.sh  # installs and migrates scheduled jobs
```
...
| [worldai_claw](https://github.com/jleechanorg/worldai_claw) | AI RPG powered by Hermes |
| [claude-commands](https://github.com/jleechanorg/claude-commands) | Claude command collection |

## License

Private — personal workspace and tools for jleechan's Hermes setup.
