...
cd ~/.smartclaw
# Create hermes.json with your real tokens (no redacted template is committed)
...
2. Verify `hermes.json` exists and has real tokens
...
# ── Gateway ───────────────────────────────────────────────────────────────────
export HERMES_URL="http://127.0.0.1:18789"
export HERMES_GATEWAY_TOKEN="<token>"           # from hermes.json
export HERMES_AO_HOOK_TOKEN="<token>"           # AO → hermes notifier webhook token
export HERMES_AO_NOTIFY_TOKEN="<token>"         # webhook daemon token for AO notifications
export HERMES_HOOKS_TOKEN="<token>"             # fallback webhook notifier token
```
...
| Variable | Required | Description |
|----------|----------|-------------|
| `HERMES_AO_HOOK_TOKEN` | Yes | Webhook token for the AO → hermes notifier. Set this so `agent-orchestrator.yaml` can authenticate to the local hermes gateway at `http://127.0.0.1:18789/hooks/agent`. |
| `HERMES_AO_NOTIFY_TOKEN` | Yes | Webhook daemon token for AO notification callbacks. |
| `HERMES_HOOKS_TOKEN` | Yes | Fallback webhook notifier token, usually matching `hermes.json` → `hooks.token`. |
...
export HERMES_AO_NOTIFY_TOKEN="<token>"
export HERMES_HOOKS_TOKEN="<token>"
```
...
|----------|---------|-------------|
| `HERMES_MONITOR_HTTP_GATEWAY_URL` | `http://127.0.0.1:18789/health` | URL for the HTTP health-check probe against the local gateway. |
...
|-----------|--------------|---------|
| [Hermes](https://github.com/hermes/hermes) | Agent runtime — persistent memory, channels, gateway | `npm install -g @jleechanorg/hermes` |
...
|--------|--------------|---------|
| [hermes-mem0](https://github.com/jleechanorg/hermes-mem0) | Long-term semantic memory via Qdrant + mem0 — auto-captures and recalls context across sessions | `hermes plugins install hermes-mem0` (already bundled in extensions/) |
...
| `hermes.json` | Live local gateway config (gitignored; must contain real tokens and runtime settings) |
...
Standalone Hermes agent config for a Discord engineering bot — separate `hermes.json` and `SOUL.md` for a Discord-channel-native agent persona.
...
Never exported:
- live config/secrets (`hermes.json*`)
