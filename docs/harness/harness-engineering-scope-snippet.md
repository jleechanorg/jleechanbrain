...
- **smartclaw / `~/.smartclaw`**: Use the **`hermes-harness`** skill in that repo for gateway, canary, deploy, and lane-backlog triage. Tracked user-scope copies for drift control live under **`docs/harness/`** in [smartclaw](https://github.com/jleechanorg/smartclaw).
```
