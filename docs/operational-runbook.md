...
- **The correct fix**: reduce both in `config.yaml`, then restart. Restart alone only clears the counter — sessions re-block immediately.
...
import json; path="$HOME/.smartclaw/config.yaml"
...
## config.yaml mutation safety

**NEVER rewrite the entire `config.yaml` file.** Always use targeted key updates:
...
Full rewrites silently drop config sections not present in the current Python scope (e.g. `agents.defaults.heartbeat` disappeared when model was updated this way on 2026-03-23). After any `config.yaml` write, verify critical keys survived: `agents.defaults.heartbeat`, `gateway.auth`, `models.providers`.
...
- **Backup `config.yaml` before upgrade**: `cp ~/.smartclaw/config.yaml ~/.smartclaw/config.yaml.pre-upgrade-$(date +%s)`
- **MANDATORY before ANY restart**: verify `meta.lastTouchedVersion` in `~/.smartclaw-consensus/config.yaml` matches running binary version — mismatch causes infinite AJV recursion → `RangeError: Maximum call stack size exceeded` crash (incident 2026-03-30). Run `bash ~/.smartclaw/scripts/gateway-preflight.sh --fix` to auto-correct.
...
sleep 5
curl -fsS -m 5 http://127.0.0.1:8644/health
```
...
1. `config.yaml` — surgical key updates only (see "config.yaml mutation safety" above)
...
PRs that touch gateway-loaded files MUST be tested against an isolated hermes gateway instance running from the PR worktree — not the live `~/.smartclaw/` gateway on port 8643.
