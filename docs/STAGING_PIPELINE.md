...
Before this pipeline, changes to `~/.smartclaw/` (the live gateway config) went straight to production with no safety gate. A bad `hermes.json` edit could crash the gateway, drop Slack messages, or break all agent sessions. Manual review caught some cases, but not consistently.
...
**What it does**: Fires on every PR (`pull_request` event). Detects whether `hermes.json` changed in the PR diff. If so, runs the full canary (which also reads `package.json` for Check 5 — SDK protocol version). The `staging-canary-gate` does not trigger on `package.json` changes alone (no hermes.json changed); the `skeptic-gate` still reviews all PRs.
...
**What it does**: When `hermes.json` changes in a PR, runs **`scripts/staging-canary.sh` (all 6 checks)** on a **self-hosted runner** (typically your Mac with `ai.smartclaw.staging` listening on port **18810**).
...
**Setup**: `.github/SELFHOSTED_TEST.md` — add labels `self-hosted` **and** `hermes-staging`. Optional repo variables: `STAGING_CANARY_PORT` (default `18810`), `HERMES_STAGING_CONFIG` (absolute path if your staging JSON is not `~/.smartclaw/hermes.staging.json`). Set `ENABLE_STAGING_CANARY_SELFHOSTED=false` to skip the job (runner offline) without removing the runner.

**Not included**: `monitor-agent.sh` stays on LaunchAgent (`ai.smartclaw.monitor-agent`); it is operational monitoring, not a PR gate.

**CI gate behavior**:
- PRs touching `hermes.json` are blocked from merging if CI fails (the `staging-canary-gate` does not trigger on `package.json` changes alone; `skeptic-gate` still reviews all PRs)
...
The staging gateway is a separate process from the production gateway (port 18789). It shares the same binary and Node.js but uses `~/.smartclaw/staging/hermes.json` as its config.
...
#    e.g., hermes.json, agents/, skills/, SOUL.md, etc.
...
### Gateway config change (hermes.json, agents/, skills/)
