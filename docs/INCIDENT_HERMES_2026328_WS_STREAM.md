...
3. **Updated config**: `meta.lastTouchedVersion` → `2026.3.24` in hermes.json
...
2. **`checkCompatibility` is NOT a valid hermes.json key** at gateway or agents.defaults level — it only exists in the mem0 vectorStore config. Setting it crashes the gateway on config reload.
