...
|-------|-------|----------|--------|---------|
| `ai.smartclaw.gateway.plist` | `ai.smartclaw.gateway` | KeepAlive | `hermes` CLI | Hermes gateway daemon (port 18789) |
...
1. **Token/secrets**: Some plists reference tokens in `~/.smartclaw/hermes.json`.
   The `install-hermes-launchd.sh` does NOT set up tokens — ensure `hermes.json`
...
- [ ] `~/.smartclaw/hermes.json` — tokens and secrets, gitignored
  - Never commit this file
