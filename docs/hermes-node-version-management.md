# Hermes Node Version Management

## Problem: Dual Node ABI Mismatch

Hermes's gateway uses a hardcoded Node binary path in the launchd plist
(`~/Library/LaunchAgents/ai.smartclaw.prod.plist`). The `hermes-mem0` extension
...
python3 -c "
import plistlib
with open('$HOME/Library/LaunchAgents/ai.smartclaw.prod.plist', 'rb') as f:
    d = plistlib.load(f)
...
1. `HERMES_GATEWAY_NODE` env override
...
- If an upgrade is needed, update the gateway plist after: `hermes gateway install --force`
...
`docker/` in this repo contains a container-based hermes setup:
- `Dockerfile` — `node:22-slim` base, installs hermes
- `entrypoint.sh` — onboards on first run, patches config.yaml, starts gateway on `0.0.0.0:8643`
- `patch.py` — surgical config.yaml patcher (tokens, bind address)
