...
(`~/Library/LaunchAgents/ai.smartclaw.gateway.plist`). The `hermes-mem0` extension
...
- `entrypoint.sh` — onboards on first run, patches hermes.json, starts gateway on `0.0.0.0:18789`
- `patch.py` — surgical hermes.json patcher (tokens, bind address)
