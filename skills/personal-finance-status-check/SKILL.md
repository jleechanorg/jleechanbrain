...
- **`/roadmap` 48h sweep is engineering-only.** The `roadmap` SKILL.md says "for each `channel_id` in scope (configured in `references/scoped-channels.txt`)… pull thread history." That's the PR/Slack-audit pattern. For personal-finance, the nextsteps doc + `~/budget/` + beads are the analog, not slack.
...
- `roadmap` (user-owned, NOT curator-managed) — covers the 48h slack sweep + PR-audit pattern for engineering. Companion, not parent.
...
- The companion `roadmap` SKILL.md (user-owned) could be extended with a § "Personal-finance status check" subsection that points to this skill — patch attempt refused 2026-08-16 because `roadmap` is user-owned.
