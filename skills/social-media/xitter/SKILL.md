...
If you already keep your X credentials in `~/.smartclaw/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.smartclaw/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.smartclaw/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
