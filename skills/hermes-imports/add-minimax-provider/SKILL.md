...
在 `~/.smartclaw/config.yaml` 的 `models.providers` 下添加 provider。根据你的接入方式选择一种或两种都配。
...
```bash
python3 -c "import json; json.load(open('$HOME/.smartclaw/config.yaml')); print('JSON OK')"
```
