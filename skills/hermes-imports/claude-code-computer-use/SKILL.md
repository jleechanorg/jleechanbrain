...
- Hermes agent auth should remain OAuth-based.
- If visual analysis fails with `OPENAI_API_KEY not found`, that is specific to optional Peekaboo `--analyze` image analysis backend, not the Hermes agent auth path.
