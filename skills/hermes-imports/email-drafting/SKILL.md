...
- `scheduling_cc_name` — first name of scheduling assistant (used in body, e.g. "Connecting X to find a time.")
- `workspace` — absolute path to Hermes workspace (for style guides and audit logs)
...
6. **Humanize** — before finalizing any draft, review it against `~/executive-assistant-skills/skills/humanizer/SKILL.md`. Remove AI-writing markers: inflated symbolism, promotional tone, em-dash overuse, "delve"/"leverage"/"foster" vocabulary, rule-of-three patterns. Email-specific rules (no dashes, signature, brevity) take precedence over humanizer suggestions if they conflict.
