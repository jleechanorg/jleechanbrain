...
### Class C — DM channel
`${SLACK_CHANNEL_ID}` is not in `config.yaml` channel maps; the script appends `$JLEECHAN_DM_CHANNEL` (default `${SLACK_CHANNEL_ID}`) to the scan list after resolving C-channels.

### Class D — `#all-jleechan-ai` (`${SLACK_CHANNEL_ID}`)
Not in the default C-channel fallback (`${SLACK_CHANNEL_ID}` + `C0AJQ5M0A0Y`); add IDs via `config.yaml` or `DROP_CHANNELS`. When `${SLACK_CHANNEL_ID}` is in the resolved channel list, it is **skipped** unless you override `DROP_EXCLUDE_CHANNELS` (default is that ID, to avoid nudging a high-churn channel). Clearing exclusions: `DROP_EXCLUDE_CHANNELS=""` excludes **nothing** (the script handles empty explicitly, unlike bash `:-`).
