...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.smartclaw")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".smartclaw")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("HERMES_HOME", Path.home() / ".smartclaw"))
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.smartclaw/prefill.json."""
