...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.smartclaw")), "skills/red-teaming/godmode/scripts/load_godmode.py")
)).read())
...
1. **Reads `~/.smartclaw/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.smartclaw/prefill.json`
...
Then set in `~/.smartclaw/config.yaml`:
```yaml
agent:
...
exec(open(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.smartclaw")), "skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.smartclaw")), "skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit `~/.smartclaw/.env`. Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.smartclaw/.env"))`
