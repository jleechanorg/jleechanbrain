...
在 `~/.smartclaw/openclaw.json` 的 `models.providers` 下添加 provider。根据你的接入方式选择一种或两种都配。
...
```bash
python3 -c "import json; json.load(open('$HOME/.smartclaw/openclaw.json')); print('JSON OK')"
```
...
# 等 3 秒后确认状态
sleep 3
launchctl print gui/$(id -u)/ai.smartclaw.gateway | grep -E "job state|last exit"
```
...
```bash
tail -20 ~/.smartclaw/logs/gateway.err.log
```
...
- **最可能原因**：配置中有非法字段
- **诊断**：`tail -20 ~/.smartclaw/logs/gateway.err.log`，找 `Unrecognized key`
...
- **诊断**：检查 `~/.smartclaw/logs/gateway.log` 中的 OAuth 相关错误
