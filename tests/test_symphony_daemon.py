...
def test_enqueue_script_resolves_mise_from_env_or_metadata() -> None:
    ...
    assert "Application Support/jleechanbrain/symphony_daemon" in enqueue_script
...
def test_sym_dispatch_and_install_use_private_runtime_default() -> None:
    ...
    assert "Application Support/jleechanbrain/symphony_daemon" in dispatch_script
    assert "Application Support/jleechanbrain/symphony_daemon" in install_script
    assert "/tmp/jleechanbrain/symphony_daemon" not in dispatch_script
    assert "/tmp/jleechanbrain/symphony_daemon" not in install_script


def test_setup_daemon_avoids_static_cookie_literal() -> None:
    setup_script = (REPO_ROOT / "scripts" / "setup-symphony-daemon.py").read_text(encoding="utf-8")
    assert "jleechanbrain_symphony_cookie" not in setup_script
