"""Validation tests for committed hermes JSON configs.

Guards against regressions in security-sensitive config properties:
- discord-eng-bot/hermes.json (public-facing Discord bot)
- hermes-config/hermes.json (main agent profile)
"""
...
DISCORD_CONFIG = REPO_ROOT / "discord-eng-bot" / "hermes.json"
MAIN_CONFIG = Path(
    os.environ.get("HERMES_TEST_MAIN_CONFIG_PATH", str(REPO_ROOT / "hermes.json"))
)
AUTH_PROFILES_CONFIG = Path(
    os.environ.get(
        "HERMES_TEST_AUTH_PROFILES_PATH",
...
HERMES_UPGRADE_SAFE = REPO_ROOT / "scripts" / "hermes-upgrade-safe.sh"
STAGING_CANARY_SCRIPT = REPO_ROOT / "scripts" / "staging-canary.sh"
STAGING_CONFIG = Path(
    os.environ.get(
        "HERMES_TEST_STAGING_CONFIG_PATH",
        str(Path.home() / ".smartclaw" / "hermes.staging.json"),
    )
)
...
def discord_cfg() -> dict:
    if not DISCORD_CONFIG.exists():
        pytest.skip("discord-eng-bot/hermes.json not present (gitignored)")
...
def main_cfg() -> dict:
    if not MAIN_CONFIG.exists():
        pytest.skip("hermes.json not present (gitignored — run from ~/.smartclaw/)")
...
def _is_minimal_config(cfg: dict) -> bool:
    """True if cfg only has channels+gateway (no agents, models, plugins, etc.)."""
    return "agents" not in cfg


_MINIMAL_SKIP_CLASSES: dict[str, list[str]] = {
    "TestMetaAndLogging": ["meta", "logging"],
    "TestAuthProfiles": ["auth"],
    "TestAgentDefaults": ["agents"],
    "TestToolsConfig": ["tools"],
    "TestEnvSection": ["env"],
    "TestRequiredAgents": ["agents"],
    "TestSkillsConfig": ["skills"],
    "TestApprovedConfigValues": ["agents"],
    "TestGatewaySecurity": ["_gateway_full"],
    "TestHooksConfig": ["hooks"],
    "TestSessionConfig": ["session"],
    "TestCommandsConfig": ["commands"],
    "TestMessagesConfig": ["messages"],
    "TestSlackChannelsConfig": ["_slack_full"],
    "TestMinimaxProviderConsistency": ["models"],
    "TestExecSafeBins": ["tools"],
    "TestPluginChannelConsistency": ["plugins"],
}


def _copy_hermes_subset(home: Path, relative_paths: list[str]) -> Path:
...
class TestMainCoreTools:
    ...
    def test_also_allow_includes_second_opinion_tools(self, main_cfg: dict):
        ...
        pytest.skip("hermes-mcp-adapter plugin not yet available (orch-n81y)")
...
# (if hermes-mcp-adapter supports it):
...
class TestMcpAdapterWiring:
    ...
    def test_main_mcp_adapter_has_authorization_header(self, main_cfg: dict):
        # hermes-mcp-adapter is not yet an installable plugin (orch-o3y0).
        # Skip until the plugin exists in the hermes registry.
        pytest.skip("hermes-mcp-adapter plugin not yet available (orch-o3y0)")
...
class TestLaunchAgentInstallers:
    def test_install_launchagents_uses_plist_bootstrap(self):
        """Gateway should be installed via plist bootstrap, not hermes CLI installer."""
        script_text = GATEWAY_INSTALL_SCRIPT.read_text(encoding="utf-8")
        # install_plist is the primary gateway install path; hermes CLI is not used
        assert "install_plist" in script_text, (
            "install-launchagents.sh must use install_plist() for gateway plist installation"
        )
        # The old hermes gateway install --force approach was removed in favor of plist bootstrap
        assert "hermes gateway install --force" not in script_text
    ...
    def test_install_launchagents_refreshes_runtime_startup_script(self):
        ...
        script_text = GATEWAY_INSTALL_SCRIPT.read_text(encoding="utf-8")
        assert 'install -m 755 "$CONFIG_DIR/startup-check.sh" "$HERMES_HOME/startup-check.sh"' in script_text
    ...
    def test_install_launchagents_dashboard_optin_persists_via_state_file(self):
        ...
        assert 'AGENTO_DASHBOARD_STATE_FILE="${HERMES_STATE_DIR:-${HOME}/.smartclaw}/.ao_dashboard_opt_in"' in script_text
    ...
    def test_install_launchagents_normalizes_prod_config_paths(self):
        ...
        assert "startswith($src)" in script_text
    ...
    def test_repo_gateway_plist_uses_prod_state_dir_and_logs(self):
        ...
        assert "/.smartclaw_prod/hermes.json" in plist_text
        assert "<key>HERMES_STATE_DIR</key>" in plist_text
        assert "/.smartclaw_prod/logs/gateway.log" in plist_text
        assert "/.smartclaw_prod/logs/gateway.err.log" in plist_text
    ...
    def test_startup_check_script_resolves_hermes_without_login_shell(self):
        ...
        assert "resolve_hermes_bin()" in script_text
        assert ".nvm/versions/node/current/bin" in script_text
        assert "/opt/homebrew/bin/hermes" in script_text
...
class TestAoRuntimeConfig:
    ...
    def test_ao_manager_defaults_to_rendered_runtime_config(self, tmp_path: Path):
        home = tmp_path / "home"
        _write_login_shell_home(home)
        repo_dir = _copy_hermes_subset(home, ["scripts/ao-manager.sh"])
    ...
    def test_install_ao_orchestrators_defaults_to_rendered_runtime_config(self, tmp_path: Path):
        ...
        _write_login_shell_home(
            ...
            exports={
                ...
                "HERMES_AO_HOOK_TOKEN": "hook-test-token",
            },
        )
        repo_dir = _copy_hermes_subset(
            home,
...
class TestSlackEnabled:
    ...
    def test_slack_channel_is_enabled(self, main_cfg: dict):
        ...
        HERMES_SYNC_ALLOW_SLACK_DISABLE guard). Skip if disabled here; the sync
...
# strings by Hermes, not expanded from env — causing silent auth failures.
# ---------------------------------------------------------------------------
...
class TestModelProviderApiKeys:
    ...
    def test_all_api_keys_are_env_refs(self, models_path: Path):
        ...
        assert not bad, (
            ...
            "Wrap as ${VAR_NAME} so Hermes expands from env (ORCH-sl0b)"
        )
...
class TestSlackMcpServerConfig:
    # hermes-mcp-adapter plugin not yet available — all tests skipped (orch-5mk8, orch-o3y0)
    ...
    def test_slack_mcp_server_present(self, main_cfg: dict):
        pytest.skip("hermes-mcp-adapter plugin not yet available (orch-5mk8)")
...
class TestSlackMcpToolsAllowed:
    def test_also_allow_includes_all_slack_mcp_tools(self, main_cfg: dict):
        pytest.skip("hermes-mcp-adapter plugin not yet available (orch-zeyi)")
...
class TestOpsScriptRegressions:
    def test_repo_claude_requires_live_config_paths_for_harness_checks(self):
        ...
        assert "HERMES_STATE_DIR" in claude_text
        assert "HERMES_CONFIG_PATH" in claude_text
        assert "never assume repo-root `~/.smartclaw/hermes.json`" in claude_text
    ...
    def test_gateway_preflight_detects_canonical_gateway_plist_wiring(self):
        ...
        assert "EnvironmentVariables.HERMES_STATE_DIR" in script_text
        assert "EnvironmentVariables.HERMES_CONFIG_PATH" in script_text
    ...
    def test_deploy_preserves_prod_config_when_staging_main_config_is_stub(self):
        ...
        assert "preserving existing prod hermes.json" in script_text
        assert "Stage 3: Config Sync" in script_text

    def test_hermes_upgrade_safe_uses_staging_launchagent_not_deleted_helper(self):
        """Safe upgrade should reuse the maintained staging launch agent path."""
        script_text = HERMES_UPGRADE_SAFE.read_text(encoding="utf-8")
    ...
    def test_staging_launchagent_executes_hermes_bin_directly(self):
        """Staging launchd should execute HERMES_BIN directly, not `node <hermes>`."""
        staging_plist = REPO_ROOT / "launchd" / "ai.smartclaw-staging.plist"
        if not staging_plist.exists():
            pytest.skip("ai.smartclaw-staging.plist not present in this checkout")
        plist_text = staging_plist.read_text(
            encoding="utf-8"
        )
        assert "<string>@HERMES_BIN@</string>" in plist_text
        assert "<string>@NODE_PATH@</string>" not in plist_text
        # Verify staging HERMES_HOME points to ~/.smartclaw (not ~/.smartclaw_prod)
        assert "<key>HERMES_HOME</key>" in plist_text
        assert "<string>@HOME@/.smartclaw</string>" in plist_text
        assert "@HOME@/.smartclaw_prod" not in plist_text
    ...
    def test_install_launchagents_prefers_staging_specific_slack_tokens(self):
        ...
        script_text = GATEWAY_INSTALL_SCRIPT.read_text(encoding="utf-8")
        assert 'HERMES_STAGING_SLACK_BOT_TOKEN' in script_text
        assert 'HERMES_STAGING_SLACK_APP_TOKEN' in script_text
        ...
        assert 'staging_overrides.setdefault("env", {})["HERMES_SLACK_APP_TOKEN"] = staging_slack_app_token' in script_text
    ...
    def test_deploy_gateway_recovery_waits_with_bounded_polling(self):
        ...
        assert "HERMES_DEPLOY_GATEWAY_START_TIMEOUT_SECONDS" in script_text
        assert "HERMES_DEPLOY_GATEWAY_START_POLL_SECONDS" in script_text
    ...
    def test_deploy_uses_canary_retry_helper_for_initial_and_post_monitor_checks(self):
        ...
        assert 'CANARY_MAX_ATTEMPTS="${HERMES_DEPLOY_CANARY_MAX_ATTEMPTS:-3}"' in script_text
        assert 'CANARY_RETRY_COOLDOWN_SECONDS="${HERMES_DEPLOY_CANARY_RETRY_COOLDOWN_SECONDS:-15}"' in script_text
    ...
    def test_deploy_uses_staging_monitor_profile_for_stub_config(self):
        ...
        assert 'HERMES_MONITOR_LOG_FILE="$STAGING_MONITOR_LOG"' in script_text
        assert 'HERMES_MONITOR_LOCK_DIR="$STAGING_MONITOR_LOCK"' in script_text
        assert "HERMES_MONITOR_TOKEN_PROBES_ENABLE=0" in script_text
        assert "HERMES_MONITOR_MEMORY_LOOKUP_ENABLE=0" in script_text
        assert "HERMES_MONITOR_DOCTOR_SH_ENABLE=0" in script_text
        assert "HERMES_MONITOR_INFERENCE_PROBE_ENABLE=0" in script_text
        assert "HERMES_MONITOR_PHASE2_ENABLE=0" in script_text
        assert 'HERMES_MONITOR_SLACK_TARGET=""' in script_text
        assert 'HERMES_MONITOR_FAILURE_SLACK_TARGET="$MONITOR_FAILURE_SLACK_TARGET"' in script_text
        assert "HERMES_MONITOR_SLACK_READ_PROBE_ENABLE=0" in script_text
        assert "HERMES_MONITOR_GATEWAY_PROBE_MESSAGE_ENABLE=0" in script_text
        assert "HERMES_MONITOR_THREAD_REPLY_CHECK=0" in script_text
        assert "HERMES_MONITOR_FAIL_CLOSED_CONFIG_SIGNATURES_ENABLE=0" in script_text
    ...
    def test_mem0_plugin_degrades_invalid_oss_fact_output_to_noop(self):
        ...
        plugin_text = (REPO_ROOT / "extensions" / "hermes-mem0" / "index.ts").read_text(
            encoding="utf-8"
        )
    ...
    def test_staging_canary_targets_staging_config_file(self):
        """Staging canary should read ~/.smartclaw/hermes.staging.json for port 18810."""
        script_text = STAGING_CANARY_SCRIPT.read_text(encoding="utf-8")
        assert 'CONFIG_FILE="$HOME/.smartclaw/hermes.staging.json"' in script_text
        assert "defaulting to staging config (~/.smartclaw/hermes.staging.json)" in script_text
    ...
    def test_monitor_phase2_uses_supported_timeout_flag(self):
        ...
        # Phase2 may use either 'hermes agent --timeout' or 'timeout <secs> ai_orch' —
    ...
    def test_monitor_thread_probe_treats_app_messages_as_bot(self):
        ...
        assert 'select((.bot_id // "") != "" or (.subtype // "") == "bot_message"' in script_text, (
            ...
            "to count Hermes replies correctly"
        )

    def test_monitor_thread_probe_resolves_bot_token_from_config(self):
        """thread probe should resolve bot token from ~/.smartclaw/hermes.json when env is missing."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_monitor_memory_probe_falls_back_to_mem0_surface(self):
        """monitor memory probe must support both `hermes memory` and older `hermes mem0` CLIs."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
        assert 'hermes memory search "test"' in script_text
        assert 'hermes mem0 search "test"' in script_text
        assert "Did you mean mem0" in script_text

    def test_run_scheduled_job_uses_supported_agent_surface(self):
        """scheduled-job wrapper must use the current hermes agent flags."""
    ...
    def test_monitor_supports_disabling_token_probes(self):
        ...
        assert 'TOKEN_PROBES_ENABLED="${HERMES_MONITOR_TOKEN_PROBES_ENABLE:-1}"' in script_text
    ...
    def test_monitor_supports_disabling_slack_read_probe(self):
        ...
        assert 'SLACK_READ_PROBE_ENABLED="${HERMES_MONITOR_SLACK_READ_PROBE_ENABLE:-1}"' in script_text
        assert 'slack read probe disabled (HERMES_MONITOR_SLACK_READ_PROBE_ENABLE=0)' in script_text
    ...
    def test_monitor_slack_e2e_matrix_covers_all_delivery_modes(self):
        ...
        assert 'SLACK_E2E_MATRIX_ENABLED="${HERMES_MONITOR_SLACK_E2E_MATRIX_ENABLE:-1}"' in script_text
        assert 'SLACK_E2E_CHANNEL_TARGET="${HERMES_MONITOR_SLACK_E2E_CHANNEL_TARGET:-${SLACK_CHANNEL_ID}}"' in script_text
        assert 'SLACK_E2E_THREAD_CHANNEL_TARGET="${HERMES_MONITOR_SLACK_E2E_THREAD_CHANNEL_TARGET:-C0AJ3SD5C79}"' in script_text
        for mode in (
            "dm_no_mention",
            "dm_with_mention",
            "channel_no_mention",
    ...
    def test_monitor_slack_e2e_matrix_prefers_nonignored_sender_identity(self):
        ...
        assert 'HERMES_MONITOR_E2E_SLACK_TOKEN' in script_text
        assert 'HERMES_SLACK_USER_TOKEN' in script_text
    ...
    def test_monitor_slack_e2e_matrix_opens_dm_with_bot_user(self):
        """DM probe should open an IM conversation with the resolved Hermes bot user."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_monitor_slack_e2e_matrix_supports_separate_thread_channel_target(self):
        ...
        assert 'HERMES_MONITOR_SLACK_E2E_THREAD_CHANNEL_TARGET' in script_text
        assert 'SLACK_E2E_THREAD_CHANNEL_TARGET="${HERMES_MONITOR_SLACK_E2E_THREAD_CHANNEL_TARGET:-C0AJ3SD5C79}"' in script_text
    ...
    def test_monitor_memory_probe_prefers_mem0_for_mem0_slot(self):
        ...
        assert 'memory_cmd=\'hermes mem0 search "test"\'' in script_text

    def test_monitor_resolves_staging_config_without_repo_root_fallback(self):
        """monitor must resolve config from HERMES env/state and never assume repo-root hermes.json."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
        ...
        assert "${HERMES_STATE_DIR}/hermes.staging.json" in script_text
        assert "$HOME/.smartclaw/staging" not in script_text
        assert "$MONITOR_REPO_ROOT/hermes.json" not in script_text
    ...
    def test_monitor_token_probes_fallback_to_base_config_for_skeletal_staging_overlay(self):
        ...
        assert 'openai_token="$(resolve_secret_ref "$(jq -r \'.plugins.entries."hermes-mem0".config.oss.embedder.config.apiKey // empty\' "$token_cfg"' in script_text
    ...
    def test_monitor_phase1_restart_is_gated_to_connectivity_failures(self):
        ...
        assert 'WS_CHURN_RESTART_ENABLED="${HERMES_MONITOR_WS_CHURN_RESTART_ENABLE:-0}"' in script_text
    ...
    def test_monitor_send_report_fail_closes_on_cli_output_signatures(self):
        ...
        assert 'send_output="$(curl' in script_text
        assert "has_fail_closed_config_parse_signature" in script_text
    ...
    def test_monitor_supports_toggling_fail_closed_config_signature_gate(self):
        ...
        assert 'FAIL_CLOSED_CONFIG_SIGNATURES_ENABLED="${HERMES_MONITOR_FAIL_CLOSED_CONFIG_SIGNATURES_ENABLE:-1}"' in script_text
    ...
    def test_qdrant_start_script_accepts_colima_fallback(self):
        ...
        assert "HERMES_QDRANT_DOCKER_CONTEXT" in script_text

    def test_qdrant_install_script_honors_context_override(self):
        """qdrant installer should honor HERMES_QDRANT_DOCKER_CONTEXT for non-default runtimes."""
    ...
    def test_doctor_memory_probe_prefers_mem0_for_mem0_slot(self):
        ...
        assert 'if [[ "$_mem_slot" == "hermes-mem0" ]]; then' in script_text
        assert 'timeout 30 hermes mem0 search "test"' in script_text
    ...
    def test_doctor_pytest_targets_live_config_path(self):
        ...
        assert 'LIVE_CONFIG_PATH="${HERMES_CONFIG_PATH:-$LIVE_HERMES/hermes.json}"' in script_text
        assert 'PYTEST_MAIN="$LIVE_HERMES/hermes.json"' in script_text
        assert 'HERMES_TEST_MAIN_CONFIG_PATH="$PYTEST_MAIN"' in script_text
        assert "TestWsSafeAgentDefaults" in script_text

    def test_doctor_port_checks_prefer_explicit_live_config_path(self):
        """doctor should honor HERMES_CONFIG_PATH when staging and prod coexist."""
        script_text = (REPO_ROOT / "scripts/doctor.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_doctor_infers_staging_port_from_profile_when_overlay_is_skeletal(self):
        """doctor should infer staging's canonical port when HERMES_CONFIG_PATH points to a minimal overlay."""
        script_text = (REPO_ROOT / "scripts/doctor.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_gateway_preflight_blocks_live_config_version_drift(self):
        ...
        assert ".smartclaw_prod/hermes.json" in script_text
        assert ".smartclaw/hermes.json" in script_text
    ...
    def test_lifecycle_launchd_templates_use_direct_ao_wrapper(self):
        ...
        assert 'ao lifecycle-worker "$project"' in manager_template
...
#
# Missing safeBinProfiles causes hermes to refuse to exec any shell command,
# producing the misleading "shell isn't working in this session" error.
# Triggered by hermes upgrades that add new safeBinProfiles enforcement.
# ---------------------------------------------------------------------------
...
class TestExecSafeBins:
    def test_every_safe_bin_has_a_profile(self, main_cfg: dict):
        ...
        Without a profile, hermes silently refuses to exec the binary, causing
        all shell tool calls to fail with 'shell isn't working in this session' (ORCH-exec1).
        Fix: run 'hermes doctor --fix' to scaffold missing profiles.
        """
        ...
        assert not missing, (
            ...
            "Run 'hermes doctor --fix' to scaffold them (ORCH-exec1)"
        )
    ...
    def test_safe_bin_profiles_exists(self, main_cfg: dict):
        ...
        if safe_bins:
            assert profiles_raw is not None, (
                ...
                "Run 'hermes doctor --fix' to initialize it (ORCH-exec1)"
            )
...
#
# meta.lastTouchedVersion is stamped by `hermes doctor` — a null or missing
# value means the config was never validated by the hermes CLI.
# ---------------------------------------------------------------------------
...
class TestMetaAndLogging:
    def test_meta_last_touched_version_present(self, main_cfg: dict):
        ...
        If this is null/missing, the config was never run through `hermes doctor`
        and may be stale or corrupted (ORCH-meta1).
        """
        version = main_cfg.get("meta", {}).get("lastTouchedVersion")
        assert version is not None, (
            "meta.lastTouchedVersion is missing/null — run 'hermes doctor' to stamp it (ORCH-meta1)"
        )
    ...
    def test_logging_level_is_valid(self, main_cfg: dict):
        """logging.level must be one of the supported hermes log levels.

        An invalid level causes hermes to fall back to a default, potentially
    ...
    def test_logging_redact_sensitive_is_valid(self, main_cfg: dict):
        ...
        An invalid value may cause hermes to log raw secrets to disk/console,
...
class TestAuthProfiles:
    ...
    def test_required_profiles_present(self, main_cfg: dict, auth_profiles_cfg: dict):
        ...
        Runtime may source profiles from either hermes.json auth.profiles or
        ...
        assert not missing, (
            ...
            "Add them with 'hermes agents auth' (ORCH-auth1)"
        )
    ...
    def test_all_profiles_have_valid_mode(self, main_cfg: dict, auth_profiles_cfg: dict):
        ...
        An invalid mode causes hermes to reject the profile at startup,
...
class TestAgentDefaults:
    ...
    def test_sandbox_mode_is_valid(self, main_cfg: dict):
        ...
        An invalid sandbox mode causes hermes to refuse to start agent sessions
    ...
    def test_workspace_access_is_valid(self, main_cfg: dict):
        ...
        An invalid workspaceAccess value causes hermes to silently block
    ...
    def test_max_concurrent_is_positive_int(self, main_cfg: dict):
        ...
        hermes to fall back to a hardcoded default (ORCH-agent1).
        """
        max_concurrent = (
            main_cfg.get("agents", {})
            .get("defaults", {})
...
def _skip_ws_safe_if_skeletal_agent_defaults(request) -> None:
    ...
    if not isinstance(d.get("timeoutSeconds"), int) or not isinstance(d.get("maxConcurrent"), int):
        pytest.skip(
            "Skeletal hermes.json (agents.defaults timeout/maxConcurrent missing) — "
            "ORCH-ws1 enforced when a full live config is present (e.g. ~/.smartclaw_prod/hermes.json)"
        )


@pytest.fixture(autouse=True)
def _skip_if_minimal_config(request) -> None:
    """Skip test classes that require sections absent from a minimal (channels+gateway-only) config."""
    cls = getattr(request, "cls", None)
    if cls is None:
        return
    required_sections = _MINIMAL_SKIP_CLASSES.get(cls.__name__)
    if required_sections is None:
        return
    main_cfg = request.getfixturevalue("main_cfg")
    if "_gateway_full" in required_sections:
        if main_cfg.get("gateway", {}).get("bind") is None:
            pytest.skip(
                "Minimal hermes.json (gateway lacks bind/mode/auth) — "
                f"{cls.__name__} requires full gateway security config"
            )
        return
    if "_slack_full" in required_sections:
        slack = main_cfg.get("channels", {}).get("slack", {})
        if "mode" not in slack or slack.get("channels", {}).get("*") is None:
            pytest.skip(
                "Minimal hermes.json (slack lacks mode/channels.*) — "
                f"{cls.__name__} requires full Slack config"
            )
        return
    missing = [s for s in required_sections if s not in main_cfg]
    if missing:
        pytest.skip(
            f"Minimal hermes.json (missing {missing}) — "
            f"{cls.__name__} requires {required_sections}"
        )


class TestWsSafeAgentDefaults:
    """Enforce CLAUDE.md protected keys against live hermes.json (doctor pytest gate)."""
    ...
    def test_plugins_memory_slot_is_hermes_mem0(self, main_cfg: dict):
        slot = (main_cfg.get("plugins", {}).get("slots") or {}).get("memory")
        assert slot == "hermes-mem0", (
            f"plugins.slots.memory={slot!r} must be 'hermes-mem0' (ORCH-ws1)"
        )
...
def _collect_hermes_model_ids(cfg: dict) -> list[str]:
...
class TestMinimaxProviderConsistency:
    def test_doctor_does_not_hardcode_stale_minimax_plugin_rename(self):
        """doctor must not force a specific MiniMax plugin id across Hermes releases."""
        script_text = (REPO_ROOT / "scripts/doctor.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_primary_minimax_model_matches_registered_provider(self, main_cfg: dict):
        ...
        if _is_minimal_config(main_cfg):
            pytest.skip("Minimal config (no agents/models) — minimax provider checks N/A")
    ...
    def test_minimax_runtime_provider_uses_anthropic_messages_endpoint(self, main_cfg: dict):
        ...
        if _is_minimal_config(main_cfg):
            pytest.skip("Minimal config (no models) — minimax endpoint checks N/A")
        providers = (main_cfg.get("models") or {}).get("providers") or {}
    ...
    def test_minimax_model_ids_match_models_providers(self, main_cfg: dict):
        ...
        if _is_minimal_config(main_cfg):
            pytest.skip("Minimal config (no agents/models) — model id checks N/A")
        providers = (main_cfg.get("models") or {}).get("providers") or {}
        for mid in _collect_hermes_model_ids(main_cfg):
            if "/" not in mid:
                continue
    ...
    def test_prod_live_config_uses_prod_workspace_and_agent_dirs_when_present(self):
        ...
        prod_cfg_path = Path.home() / ".smartclaw_prod" / "hermes.json"
        ...
        if _is_minimal_config(prod_cfg):
            pytest.skip("Minimal prod config (no agents section) — workspace/agentDir checks N/A")
    ...
    def test_all_agent_entries_define_workspace_and_agent_dir(self, main_cfg: dict):
        ...
        if _is_minimal_config(main_cfg):
            pytest.skip("Minimal config (no agents) — agent entry checks N/A")
        bad_agents = []
...
class TestToolsConfig:
    ...
    def test_exec_ask_is_off(self, main_cfg: dict):
        ...
        When ask='always' or 'untrusted', hermes pauses to request human
    ...
    def test_web_search_provider_is_set(self, main_cfg: dict):
        ...
        An empty provider string causes hermes to silently fail all web
...
#
# Raw secrets in hermes.json are committed to git (via the redacted copy)
# and also logged by hermes when logging.redactSensitive is not 'all'.
# ---------------------------------------------------------------------------
...
class TestEnvSection:
    def test_env_section_present(self, main_cfg: dict):
        """env section must exist in hermes.json.
    ...
    def test_env_values_are_not_raw_credential_literals(self, main_cfg: dict):
        ...
        Raw credential literals in env section are logged when hermes logs
        config state, creating credential leak risk in log files (ORCH-env1).
        """
        pytest.skip(
            "hermes.json is a local gitignored runtime file and may contain real tokens; "
...
class TestGatewaySecurity:
    def test_gateway_port_is_valid(self, main_cfg: dict):
        """gateway.port must be a valid port number.

        Port 8642 is the canonical hermes gateway port (migrated from legacy 18789).
        Changing the port requires coordinated updates to all callers (ORCH-gw1).
        """
        port = main_cfg.get("gateway", {}).get("port")
        assert port in (8642, 18789), (
            f"gateway.port={port!r} — expected 8642 (or legacy 18789). "
...
#
# Invalid enum values cause hermes to ignore the setting and apply defaults,
...
class TestCommandsConfig:
    ...
    def test_commands_native_is_valid(self, main_cfg: dict):
        ...
        An invalid value causes hermes to fall back to a default, which may
...
class TestPluginChannelConsistency:
    ...
    def test_channel_plugin_enabled_states_consistent(self, main_cfg: dict):
        ...
        hermes to try to connect a channel whose plugin is not loaded,
...
class TestSlackChannelsConfig:
    ...
    def test_slack_group_policy_is_valid(self, main_cfg: dict):
        ...
        An invalid groupPolicy value causes hermes to fall back to default
    ...
    def test_slack_thread_history_scope_is_channel(self, main_cfg: dict):
        ...
        misses non-mention thread replies (hermes/hermes#29657).
        """
    ...
    def test_slack_wildcard_channel_allows_all_invited(self, main_cfg: dict):
        ...
        so Hermes listens to every invited channel without per-ID maintenance (ORCH-slack3).
        """
        slack = self._slack_cfg(main_cfg)
...
class TestRequiredAgents:
    ...
    def test_main_agent_entry_has_id_field(self, main_cfg: dict):
        ...
        An agent entry without an id field is silently skipped by hermes,
