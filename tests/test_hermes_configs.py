"""Validation tests for committed hermes config files.

Guards against regressions in security-sensitive config properties:
- discord-eng-bot/config.yaml (public-facing Discord bot)
- hermes-config/config.yaml (main agent profile)
"""
...
DISCORD_CONFIG = REPO_ROOT / "discord-eng-bot" / "config.yaml"
MAIN_CONFIG = Path(
    os.environ.get("HERMES_TEST_MAIN_CONFIG_PATH", str(REPO_ROOT / "config.yaml"))
)
...
STAGING_CONFIG = Path(
    os.environ.get(
        ...
        str(Path.home() / ".smartclaw" / "config.staging.yaml"),
    )
)
...
class TestOpsScriptRegressions:
    def test_repo_claude_requires_live_config_paths_for_harness_checks(self):
        ...
        assert "never assume repo-root `~/.smartclaw/config.yaml`" in claude_text
    ...
    def test_deploy_preserves_prod_config_when_staging_main_config_is_stub(self):
        ...
        assert "preserving existing prod config.yaml" in script_text
    ...
    def test_staging_launchagent_executes_hermes_bin_directly(self):
        ...
        assert "<string>@HOME@/.smartclaw/config.staging.yaml</string>" in plist_text

    def test_install_launchagents_generates_valid_staging_overlay_for_8644(self):
        """Staging overlay generation must keep runtime schema valid and force the staging port."""
        script_text = GATEWAY_INSTALL_SCRIPT.read_text(encoding="utf-8")
        assert '"port": 8644' in script_text
        assert '"http://localhost:8644"' in script_text
        assert '"http://127.0.0.1:8644"' in script_text
    ...
    def test_deploy_gateway_recovery_waits_with_bounded_polling(self):
        ...
        assert 'launchctl start "gui/$(id -u)/ai.smartclaw.prod"' not in script_text
        assert "launchctl list ai.smartclaw.prod" not in script_text
    ...
    def test_staging_canary_targets_staging_config_file(self):
        """Staging canary should read ~/.smartclaw/config.staging.yaml for port 8644."""
        script_text = STAGING_CANARY_SCRIPT.read_text(encoding="utf-8")
        assert 'CONFIG_FILE="$HOME/.smartclaw/config.staging.yaml"' in script_text
        assert "defaulting to staging config (~/.smartclaw/config.staging.yaml)" in script_text
    ...
    def test_monitor_thread_probe_resolves_bot_token_from_config(self):
        """thread probe should resolve bot token from ~/.smartclaw/config.yaml when env is missing."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
    ...
    def test_monitor_resolves_staging_config_without_repo_root_fallback(self):
        """monitor must resolve config from HERMES env/state and never assume repo-root config.yaml."""
        script_text = (REPO_ROOT / "monitor-agent.sh").read_text(
            encoding="utf-8"
        )
        ...
        assert "${HERMES_STATE_DIR}/config.staging.yaml" in script_text
        assert "$HOME/.smartclaw/staging" not in script_text
        assert "$MONITOR_REPO_ROOT/config.yaml" not in script_text
    ...
    def test_monitor_infers_gateway_port_from_profile_when_overlay_is_skeletal(self):
        ...
        assert '8644' in script_text
        assert '8643' in script_text
    ...
    def test_doctor_pytest_targets_live_config_path(self):
        ...
        assert 'LIVE_CONFIG_PATH="${HERMES_CONFIG_PATH:-$LIVE_HERMES/config.yaml}"' in script_text
        assert 'PYTEST_MAIN="$LIVE_HERMES/config.yaml"' in script_text
    ...
    def test_doctor_uses_profile_specific_runtime_expectations(self):
        ...
        assert "staging) printf '8644'" in script_text
...
class TestAuthProfiles:
    ...
    def test_required_profiles_present(self, main_cfg: dict, auth_profiles_cfg: dict):
        ...
        Runtime may source profiles from either config.yaml auth.profiles or
...
class TestWsSafeAgentDefaults:
    """Enforce CLAUDE.md protected keys against live config.yaml (doctor pytest gate)."""
...
#
# Raw secrets in config.yaml are committed to git (via the redacted copy)
...
class TestEnvSection:
    def test_env_section_present(self, main_cfg: dict):
        """env section must exist in config.yaml.
    ...
    def test_env_values_are_not_raw_credential_literals(self, main_cfg: dict):
        ...
        """
        pytest.skip(
            "config.yaml is a local gitignored runtime file and may contain real tokens; "
...
class TestGatewaySecurity:
    def test_gateway_port_is_8643(self, main_cfg: dict):
        """gateway.port must be 8643 (the canonical hermes gateway port).

        Using a non-standard port breaks all tooling that hardcodes 8643
        ...
        assert port == 8643, (
            f"gateway.port={port!r} — expected 8643. "
