...
# synthetic Hermes bot replies in the appropriate conversation surface.
set -euo pipefail
...
cat > "$STATE_DIR/hermes.json" <<'JSON'
{
  ...
  "channels": {
    "slack": {
      "enabled": true,
      "botToken": "xoxb-hermes",
      "appToken": "xapp-hermes",
      "channels": {
...
def synth_bot_reply(ts_value: str, channel: str, thread_ts: str | None):
    ...
    return {
        ...
        "user": "UHERMES",
        "bot_id": "BHERMES",
        "bot_profile": {"user_id": "UHERMES"},
        "text": "synthetic hermes reply",
...
if url.endswith("/conversations.open"):
    if auth in ("xoxp-human", "xoxp-app-user", "xoxb-hermes") and params.get("users") == "UHERMES":
        print('{"ok":true,"channel":{"id":"DHERMES"}}')
    else:
...
if url.endswith("/chat.postMessage"):
    ...
    msg = {
        ...
        "app_id": "AHERMES" if kind in ("canary", "app_user", "bot") else "",
    }
...
HERMES_SLACK_USER_TOKEN="xoxb-canary" \
HERMES_MONITOR_CANARY_BOT_TOKEN="xoxb-canary" \
HERMES_MONITOR_BOT_USER_ID="UHERMES" \
HERMES_STATE_DIR="$STATE_DIR" \
HERMES_CONFIG_PATH="$STATE_DIR/hermes.json" \
HERMES_MONITOR_OC_DIR="$OC_DIR" \
HERMES_MONITOR_SLACK_E2E_CHANNEL_TARGET="${SLACK_CHANNEL_ID}" \
HERMES_MONITOR_SLACK_E2E_THREAD_CHANNEL_TARGET="C0AJ3SD5C79" \
...
HERMES_MONITOR_SLACK_E2E_MATRIX_ENABLE=1 \
HERMES_MONITOR_SLACK_E2E_TIMEOUT_SECONDS=2 \
HERMES_MONITOR_SLACK_E2E_POLL_INTERVAL_SECONDS=1 \
...
if grep -q 'sender=SLACK_USER_TOKEN' "$LOG_FILE"; then
  pass "monitor prefers SLACK_USER_TOKEN when HERMES_SLACK_USER_TOKEN is bot-scoped"
else
...
env -u SLACK_USER_TOKEN \
  ...
  PATH="$HOME_DIR/bin:$PATH" \
  SLACK_STATE_FILE="$SLACK_STATE_FILE" \
  HERMES_SLACK_USER_TOKEN="xoxp-human" \
  HERMES_MONITOR_CANARY_BOT_TOKEN="xoxb-canary" \
  HERMES_MONITOR_BOT_USER_ID="UHERMES" \
  HERMES_STATE_DIR="$STATE_DIR" \
  HERMES_CONFIG_PATH="$STATE_DIR/hermes.json" \
  ...
  HERMES_MONITOR_STATUS_BROADCAST_ENABLE=0 \
  HERMES_MONITOR_PHASE2_ENABLE=0 \
  HERMES_MONITOR_RUN_CANARY=1 \
  HERMES_MONITOR_SLACK_E2E_MATRIX_ENABLE=1 \
  HERMES_MONITOR_SLACK_E2E_TIMEOUT_SECONDS=2 \
  HERMES_MONITOR_SLACK_E2E_POLL_INTERVAL_SECONDS=1 \
  HERMES_MONITOR_WS_CHURN_THRESHOLD=1 \
  HERMES_MONITOR_LOG_FILE="$WS_LOG_FILE" \
  bash "$SCRIPT"
...
if grep -q 'sender=HERMES_SLACK_USER_TOKEN' "$WS_LOG_FILE"; then
  pass "monitor accepts HERMES_SLACK_USER_TOKEN as the positive probe sender"
else
  fail "monitor did not use HERMES_SLACK_USER_TOKEN for positive Slack probes"
fi
...
HERMES_MONITOR_RUN_CANARY=1 \
HERMES_MONITOR_SLACK_E2E_MATRIX_ENABLE=1 \
HERMES_MONITOR_SLACK_E2E_TIMEOUT_SECONDS=5 \
HERMES_MONITOR_SLACK_E2E_POLL_INTERVAL_SECONDS=1 \
HERMES_MONITOR_WS_CHURN_THRESHOLD=999999 \
HERMES_MONITOR_LOG_FILE="$SLOW_LOG_FILE" \
...
env -u SLACK_USER_TOKEN \
  ...
  PATH="$HOME_DIR/bin:$PATH" \
  SLACK_STATE_FILE="$SLACK_STATE_FILE" \
  HERMES_SLACK_USER_TOKEN="xoxp-app-user" \
  HERMES_MONITOR_CANARY_BOT_TOKEN="xoxb-canary" \
  HERMES_MONITOR_BOT_USER_ID="UHERMES" \
  HERMES_STATE_DIR="$STATE_DIR" \
  HERMES_CONFIG_PATH="$STATE_DIR/hermes.json" \
  ...
  HERMES_MONITOR_SLACK_E2E_POLL_INTERVAL_SECONDS=1 \
  HERMES_MONITOR_WS_CHURN_THRESHOLD=999999 \
  HERMES_MONITOR_LOG_FILE="$APP_USER_LOG_FILE" \
  bash "$SCRIPT"
...
env -u SLACK_USER_TOKEN \
  ...
  PATH="$HOME_DIR/bin:$PATH" \
  SLACK_STATE_FILE="$SLACK_STATE_FILE" \
  HERMES_SLACK_USER_TOKEN="xoxb-hermes" \
  HERMES_MONITOR_CANARY_BOT_TOKEN="xoxb-canary" \
  HERMES_MONITOR_BOT_USER_ID="UHERMES" \
  HERMES_STATE_DIR="$STATE_DIR" \
  HERMES_CONFIG_PATH="$STATE_DIR/hermes.json" \
  ...
  HERMES_MONITOR_SLACK_E2E_POLL_INTERVAL_SECONDS=1 \
  HERMES_MONITOR_WS_CHURN_THRESHOLD=999999 \
  HERMES_MONITOR_LOG_FILE="$BOT_SENDER_LOG_FILE" \
  bash "$SCRIPT"
