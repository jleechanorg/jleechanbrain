...
**Scope:** This skill applies to **`~/.smartclaw`** (the [jleechanbrain](https://github.com/jleechanorg/jleechanbrain) harness repo) and production/staging gateway operations. It **does not** replace the general protocol in **`~/.claude/skills/harness-engineering/SKILL.md`** — it **extends** it for Hermes-specific failure modes.
...
2. `bash scripts/staging-canary.sh --port 8643` for prod config path when testing prod (includes `gateway.err.log` lane-backlog probe; parity with doctor)
