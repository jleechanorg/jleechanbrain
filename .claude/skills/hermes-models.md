...
**Live config**: `~/.smartclaw/hermes.json` → `agents.defaults.model`
...
| `openrouter/auto` | ❓ NOT CONFIGURED | `api_key` → `OPENROUTER_API_KEY` | Key not in hermes.json env; add before using |
...
**Before writing to hermes.json, verify the model is available on the current plan:**

```bash
# Get the current MiniMax API key from auth-profiles
KEY=$(python3 -c "import json, pathlib; d=json.load(open(pathlib.Path.home() / '.smartclaw/agents/main/agent/auth-profiles.json')); print((d.get('profiles',{}).get('minimax:default',{}).get('key') or '').strip())")
if [ -z "$KEY" ]; then
  echo "Missing profiles.minimax:default.key in ~/.smartclaw/agents/main/agent/auth-profiles.json" >&2
...
**Never add a model to hermes.json without a 200 probe response.**
...
path = str(Path("~/.smartclaw/hermes.json").expanduser())
...
```bash
grep "agent model" /tmp/hermes/hermes-$(date +%F).log | tail -2
```
