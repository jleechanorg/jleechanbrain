...
**Live config**: `~/.smartclaw/config.yaml` → `agents.defaults.model`
...
| `openrouter/auto` | ❓ NOT CONFIGURED | `api_key` → `OPENROUTER_API_KEY` | Key not in config.yaml env; add before using |
...
**Before writing to config.yaml, verify the model is available on the current plan:**
...
**Never add a model to config.yaml without a 200 probe response.**

### Step 1 — Update config (surgical — never rewrite the whole file)

```python
import json
path = "${HOME}/.smartclaw/config.yaml"
...
```bash
kill -9 $(lsof -ti :8643); sleep 12; lsof -i :8643 | grep LISTEN
```
