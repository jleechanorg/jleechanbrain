...
**Purpose**: Install, verify, operate, and troubleshoot the Hermes staging→production deployment pipeline
**Scope**: `~/.smartclaw/` (staging, port 18810) ↔ `~/.smartclaw_prod/` (production, port 8643)
...
│  │   └── ai.smartclaw.prod.plist     → ai.smartclaw.prod          │
...
[[ -f ~/.smartclaw/config.yaml ]] && cp ~/.smartclaw/config.yaml ~/.smartclaw_prod/config.yaml
...
This:
1. Substitutes `@HOME@`, `@NODE_PATH@`, `@HERMES_BIN@`, etc. in plist templates
2. Creates `~/Library/LaunchAgents/ai.smartclaw.staging.plist`
3. Creates `~/Library/LaunchAgents/ai.smartclaw.prod.plist`
...
# Production (port 8643)
launchctl start gui/$(id -u)/ai.smartclaw.prod
sleep 20
curl -sf --max-time 8 http://127.0.0.1:8643/health
```
...
4. **Sync config** — `rsync` scripts/, workspace/, memory/, cron/ from staging → prod; `cp config.yaml`
...
# Production (port 8643)
HERMES_STAGING_CONFIG="$HOME/.smartclaw_prod/config.yaml" \
  bash ~/.smartclaw/scripts/staging-canary.sh --port 8643
```
...
```bash
# 1. Stop prod gateway
launchctl stop gui/$(id -u)/ai.smartclaw.prod
...
# git -C ~/.smartclaw show HEAD:config.yaml > ~/.smartclaw_prod/config.yaml
...
This auto-corrects `meta.lastTouchedVersion` in `~/.smartclaw-consensus/config.yaml` to match the running binary version.
...
| File | Purpose |
|---|---|
| `~/.smartclaw/config.yaml` | Staging config (port 18810) |
| `~/.smartclaw_prod/config.yaml` | Production config (port 8643) |
...
| `~/.smartclaw/launchd/ai.smartclaw.prod.plist` | Production plist template (port 8643) |
| `~/Library/LaunchAgents/ai.smartclaw.staging.plist` | Deployed staging plist |
| `~/Library/LaunchAgents/ai.smartclaw.prod.plist` | Deployed production plist |
...
| `~/.smartclaw-consensus/config.yaml` | Consensus config (AJV version tracking) |
| `~/.smartclaw_prod/config.yaml.bak.*` | Rotating backups created by `install-launchagents.sh` (not by deploy.sh) |
...
lsof -i :18810 2>/dev/null || lsof -i :8643 2>/dev/null
...
```bash
bash ~/.smartclaw/scripts/gateway-preflight.sh --fix
launchctl stop gui/$(id -u)/ai.smartclaw.prod && launchctl start gui/$(id -u)/ai.smartclaw.prod
```
...
GATEWAY_NODE="$HOME/.nvm/versions/node/v22.22.0/bin/node"
"$GATEWAY_NODE" -e "require('$HOME/.smartclaw/extensions/hermes-mem0/node_modules/better-sqlite3')" 2>/dev/null || echo "FAIL"
# If fails: rebuild
cd ~/.smartclaw/extensions/hermes-mem0
...
# Fix — convert to XML so hermes CLI / gateway status commands can parse it
plutil -convert xml1 ~/Library/LaunchAgents/ai.smartclaw.prod.plist
...
ls ~/Library/LaunchAgents/*hermes*
# Two gateways run simultaneously on different ports — both are needed:
#   ai.smartclaw.prod    (port 8643, production)
...
```bash
defaults write ~/Library/LaunchAgents/ai.smartclaw.prod.plist ThrottleInterval -int 30
launchctl kickstart -k gui/$(id -u)/ai.smartclaw.prod
```
...
### Upgrading hermes through the staging gate
...
validate_sdk_compatibility <new-hermes-version>
# If FAIL: major SDK jump — do NOT upgrade

# 2. Upgrade staging
npm install -g hermes@<new-version>
...
| `ai.smartclaw.prod` | 8643 | `~/.smartclaw_prod/` | Production gateway |
...
| `ai.smartclaw.mem0-watchdog` | — | `~/.smartclaw/` | Periodic better-sqlite3 ABI check |
...
- **Production gateway**: `launchctl print gui/$(id -u)/ai.smartclaw.prod`
- **Staging gateway**: `launchctl print gui/$(id -u)/ai.smartclaw.staging`
- **Slack health channel**: `#C0AJ3SD5C79` (Hermes design channel)
