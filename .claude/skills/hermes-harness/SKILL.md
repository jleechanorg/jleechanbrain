---
name: hermes-harness
description: smartclaw / Hermes-specific harness — gateway, canary, deploy, lane backlog, staging vs prod. Use with user-scope harness-engineering.
---

# Hermes harness (repository-local)

**Scope:** This skill applies to **`~/.smartclaw`** (the [smartclaw](https://github.com/jleechanorg/smartclaw) harness repo) and production/staging gateway operations. It **does not** replace the general protocol in **`~/.claude/skills/harness-engineering/SKILL.md`** — it **extends** it for Hermes-specific failure modes.
...
| `session file locked` | Stale `.jsonl.lock` in `$HERMES_STATE_DIR/agents/main/sessions/` | Lock cleanup + restart (see `CLAUDE.md`) |
| Staging missing, prod OK | `launchctl print gui/$UID/ai.smartclaw.staging` | Bootstrap staging plist |
| Multi-gateway orphan | `pgrep -x hermes-gateway` > expected | `deploy.sh` / `CLAUDE.md` single-instance |
...
1. `bash scripts/doctor.sh` (or `HERMES_DOCTOR_SKIP_INFERENCE=1` when appropriate)
...
- **Repo-local `/harness`** (`.claude/commands/harness.md` **in this workspace**) — short pointer + Hermes gates; **wins** for project-specific detail when both exist.
